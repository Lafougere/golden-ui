 ...
 * ====================== */
	...
	var ScrollPane = function ScrollPane(element, options) {
	...
	ScrollPane.prototype = {
		...
		onArrowUpClick: function onArrowUpClick(e) {
			this.iterations = 0;
			this.delay = 220;
			this.computeSize();
		...
		handleArrowUpHold: function handleArrowUpHold(e) {
			this.computeSize();
			this.sliceId = Math.max(0, this.sliceId - 1);
			this.$handle.css({ top: this.sliceId * this.handleIncrement });
			this.$element.animate({ scrollTop: this.contentIncrement * this.sliceId }, 200);
		...
		onArrowDownClick: function onArrowDownClick(e) {
			this.iterations = 0;
			this.delay = 220;
			this.computeSize();
		...
		handleArrowDownHold: function handleArrowDownHold(e) {
			this.computeSize();
			this.sliceId = Math.min(this.slices, this.sliceId + 1);
			this.$handle.css({ top: this.sliceId * this.handleIncrement });
			this.$element.animate({ scrollTop: this.contentIncrement * this.sliceId }, 200);
		...
		onTrackClick: function onTrackClick(e) {
		...
		updateArrowStatus: function updateArrowStatus() {
			if (this.sliceId == 0) {
		...
		computeSize: function computeSize() {
			...
			if (this.scrollHeight <= this.containerHeight) this.$parent.addClass('hide-scrollbar');else this.$parent.removeClass('hide-scrollbar');
...
//# sourceMappingURL=golden-ui-scrollpane.js.map
